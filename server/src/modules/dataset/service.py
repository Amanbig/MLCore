...
class DatasetService:
    ...
    def create_dataset(
        self, db: Session, data: DatasetRequest, token_payload: AuthToken
    ) -> DatasetResponse:
        file = self.file_service.create_file(db=db, **data.model_dump())
        self.repo.create(
            db=db,
            obj_in=DatasetRequest(**data.model_dump(), user_id=token_payload.id, file_id=file.id),
        )

    def get_dataset(
        self, db: Session, dataset_id: int, token_payload: AuthToken
    ) -> DatasetResponse:
        return self.repo.get(db=db, id=dataset_id)

    def get_datasets(self, db: Session, token_payload: AuthToken) -> List[DatasetResponse]:
        return self.repo.get_all(db=db)

    def update_dataset(
        self, db: Session, dataset_id: int, data: DatasetRequest, token_payload: AuthToken
    ) -> DatasetResponse:
